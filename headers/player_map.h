// Name: <Abdallah> <Abualkheir>
// Date: <5/14/2020>
// Course Code: CS-172-<2>
// Copyright (C) <Abdallah Abualkheir> 2020 - 2021
//derived class header file
#pragma once
#include "../headers/gameboard.h" //include base class
class Player : public Game_board  // define a Player class that is inheriting from the base class Game_board
{
public:
    //default contructor calls the base class constructor
    Player() : Game_board(10, 10) {}
    size_t count = 0;
    //input : coord1: the X axis, coord2: the Y axix, Battle_ship object, x is the orienatation of the ship((0 for vertical and 1 for horizontal))
    //function: input ships in the map depending on the input passed in the function
    void set_player_ships(int coord1, int coord2, Battle_ship b, bool x)
    {
        char ship = '0' + count;
        //check if there is already a ship at the location passed in
        // if so, exit the function so the program/user can choose again
        if (board_arry[coord1][coord2] == ship)
        {
            cout << "Invalid input, there is already a ship in that location." << endl;
            return;
        }
        // if the bool value passed is 1 or true then print a horizontal ship
        if (x)
        {
            int j = 2; // j checks how many spaces is between the imporoperly placed sign and the ship.
                       // it starts at 2 because the first space is already checked in the while loop condition
            //prints as many signs as the ship's length
            for (int i = 0; i < b.get_length(); i++)
            {
                //In case of ship's length going over the edge, check if the location the sign is being printed to, has other signs next to it
                //indicating that its a complete ship and has no gaps.
                if (coord2 + i > rows - 1) //last index that coord can reach is 9, and rows is 10. so we compare it coord with row-1 =9;
                {
                    board_arry[coord1][coord2 - i] = ship;
                    //if there is a gap, check how far is the sign from the rest of the ship
                    while (board_arry[coord1][coord2 - i + (j - 1)] != ship)
                    {
                        //if the ship is found, print the sign in the proper place then delete the false sign by replacing it with a ~
                        if (board_arry[coord1][coord2 - i + j] == ship)
                        {

                            j--;
                            board_arry[coord1][coord2 - i + j] = ship;
                            board_arry[coord1][coord2 - i] = '~';
                        }
                        //if the ship is not found, increment j to search the space next to the one searched
                        j++;
                    }
                }
                //if the ship is not near any edges, print normally
                else
                {
                    board_arry[coord1][coord2 + i] = ship;
                }
            }
        }
        // if the bool value passed is 0 or false then print a vertical ship
        if (!x)
        {
            int j = 2; // j checks how many spaces is between the imporoperly placed sign and the ship.
                       // it starts at 2 because the first space is already checked in the while loop condition
            //prints as many signs as the ship's length

            for (int i = 0; i < b.get_length(); i++)
            {
                //In case of ship's length going over the edge, check if the location the sign is being printed to has other signs next to it
                //indicating that its a complete ship and has no gaps.
                if (coord1 + i > columns - 1) //last index that coord can reach is 9, and rows is 10. so we compare it coord with row-1 =9;
                {
                    board_arry[coord1 - i][coord2] = ship;
                    //if there is a gap, check how far is the sign from the rest of the ship
                    while (board_arry[coord1 - i + (j - 1)][coord2] != ship)
                    {

                        if (board_arry[coord1 - i + j][coord2] == ship)
                        {
                            j--;
                            board_arry[coord1 - i + j][coord2] = ship;
                            board_arry[coord1 - i][coord2] = '~';
                        }
                        //if the ship is not found, increment j to search the space next to the one searched
                        j++;
                    }
                }
                else
                {
                    board_arry[coord1 + i][coord2] = ship;
                }
            }
        }
        //increment the number of the ships in the map
        ships++;
    }
    void set_player_map()
    {
        int ship_choice;  // user's choice of ship
        int location_x;   // the location on the x axis
        int location_y;   // the location on the y axix
        bool orientation; //vertical or horizontal
        cout << "place your ships!" << endl;
        do
        {
            cout << "which ship would you like to place ?" << endl;
            cout << "1- Carrier (5 holes)\n2-Cruiser (3 holes)\n3-Submarine(3 holes)\n4-Destroyer(2 holes)\n";
            cin >> ship_choice; // ask for ship choice
            cout << "Where would you like to place it? (pick the location where you want the top of your ship to be on the Y and X axis)" << endl;
            cout << "Top is 0, bottom is 9" << endl;
            cout << "Enter ships location on the X axis ";
            cin >> location_x; //ask for x location
            cout << "Enter ships location on the Y axis " << endl;
            cout << "Far left is 0, far rigt is 9 " << endl;
            cin >> location_y; //ask for y location
            cout << "Do you want your ship to be vertical or horizontal? (0 for vertical and 1 for horizontal)" << endl;
            cin >> orientation; // ask for orientation
            //bad input check
            while ((ship_choice < 0 || ship_choice > 4) || (location_x < 0 || location_x > 9) || (location_y < 0 || location_y > 9) || (orientation < 0 || location_x > 1))
            {
                cout << "You entered invalid input. please enter proper input\n";
                cout << "which ship would you like to place ?" << endl;
                cout << "1- Carrier (5 holes)\n2-Cruiser (3 holes)\n3-Submarine(3 holes)\n4-Destroyer(2 holes)\n";
                cin >> ship_choice; // ask for ship choice
                cout << "Where would you like to place it? (pick the location where you want the top of your ship to be on the Y and X axis)" << endl;
                cout << "Top is 0, bottom is 9" << endl;
                cout << "Enter ships location on the X axis ";
                cin >> location_x; //ask for x location
                cout << "Enter ships location on the Y axis " << endl;
                cout << "Far left is 0, far rigt is 9 " << endl;
                cin >> location_y; //ask for y location
                cout << "Do you want your ship to be vertical or horizontal? (0 for vertical and 1 for horizontal)" << endl;
                cin >> orientation; // ask for orientation
            }
            switch (ship_choice)
            {
            case 1:                                     // if the user choose one then create the carrier
                this->battleships.push_back(Carrier()); //add to the ships vector
                set_player_ships(location_x, location_y, this->battleships[count], orientation);
                count++;
                //show its location on the user map
                this->show_map();
                break;
            case 2: // if the user choose 2 then create the cruiser
                this->battleships.push_back(Cruiser());
                set_player_ships(location_x, location_y, this->battleships[count], orientation);
                count++;
                this->show_map();
                break;
            case 3: // if the user choose 3 then create the submarine
                this->battleships.push_back(Submarine());
                set_player_ships(location_x, location_y, this->battleships[count], orientation);
                count++;
                this->show_map();
                break;
            case 4: // if the user choose 4 then create the destroyer
                this->battleships.push_back(Destroyer());
                set_player_ships(location_x, location_y, this->battleships[count], orientation);
                count++;
                this->show_map();
                break;
            }
        } while (!this->full_map()); // keep adding ships until the map is full with 5 ships
    }
    //this is the function the opponenet is going to use to attack the player
    void attack(int x, int y)
    {
        int new_x = rand() % 10;                                                // new x locatain for the second attack function call
        int new_y = rand() % 10;                                                // new y locatain for the second attack function call
        if ((this->board_arry[x][y] == '/') || (this->board_arry[x][y] == 'X')) // if the opponent attacked an already attacked location
        {                                                                       // attack again with a new random location
            attack(new_x, new_y);
        }
        //if the program detects a sign at the location passed in, then register that as a hit and replace the # with an
        for (int i = 0; i < 5; i++)
        {
            if (this->board_arry[x][y] == '0' + i) // X to indicate a damaged ship
            {

                this->board_arry[x][y] = 'X';
                battleships[i].damage();
                cout << "Hit!" << endl;
                health_check(battleships[i]);
            }
        }
        if (this->board_arry[x][y] == '~') //otherwise, if the program detects a ~ sign indicating no ship, then register that as a miss
        {                                  // and replace the ~ by a / to indicate a miss
            this->board_arry[x][y] = '/';
            cout << "Miss!" << endl;
        }
    }
};